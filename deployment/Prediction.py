# -*- coding: utf-8 -*-
"""
Created on Thu Mar 14 11:44:43 2024
...
def add_url_to_database(url, url_type):
    ...
    try:
        ...
        st.error(f"Failed to add URL to database: {e}")
        return False
...
def main():
    ...
    if st.button("Database Check Result", disabled=modelFlag == 0):
        if input_url:
            url_type = check_url_type(input_url)
            if url_type:
                database = f"URL '{input_url}' is of type '{url_type}'."
            else:
                database = "URL does not exist in the database."
                databaseFlag = 0
                modelFlag = 0
        else:
            database = "Please enter a URL to check."
    ...
    if st.button("Model Test Result", disabled=databaseFlag == 1):
        if input_url:
            diagnosis = get_prediction_from_url(input_url)
                
            # Add URL and its type to the database
            if add_url_to_database(input_url, diagnosis):
                add = f"URL '{input_url}' added to the database with type '{diagnosis}'."
                databaseFlag = 1
                modelFlag = 1
            else:
                add = "Failed to add URL to the database."
        else:
            diagnosis = "Please enter a URL to check."
